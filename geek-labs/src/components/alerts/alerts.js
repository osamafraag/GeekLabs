...
const handeClick = (event,id)=>{
  ...
  return (
    ...
    <>
    <div style={{position:'absolute',width:'846px',height:'734px',left:'133px',top:'113px'}}>
      {ApiSample.map((sample,index)=>{
        return(
          <div key={sample.id}
          style={{marginBottom:'16px',borderRadius:'8px',backgroundColor:'var(--main-color)'}}>
          <div  onClick={(event)=>{handeClick(event,sample.id)}} style={{zIndex: '1',display:'flex',width:'846px',height:'59px',backgroundColor:selectedElement == sample.id ? 'var(--accent-color)' :'var(--secondary-color)',borderRadius:'8px'}}>
        ...
        <span  className='my-auto' style={{width:'2px',height:'36px',backgroundColor:selectedElement == sample.id ? 'white':'#414040',borderRadius:'0.5px',}}></span>
        <div  className='my-auto d-flex' style={{width:'24.8%',height:'80%'}}>
        <img className='my-auto mx-5' src={Persent}></img>
        <span className='my-auto me-5' style={{color:sample.persent >= 0 ? 'var(--green-color)':'var(--orange-color)'}}>{sample.persent}%</span>
        </div>
        <span  className='my-auto' style={{width:'2px',height:'36px',backgroundColor:selectedElement == sample.id ? 'white':'#414040',borderRadius:'0.5px',}}></span>
        <div  className='my-auto d-flex' style={{width:'24.8%',height:'80%'}}>
